...
% Note: This code was built on top of v5 of the badTrial code. BadRMS
% trials are now being incorporated to badTrials and also the same would be
% used for the thirty percent cutoff for finding the badElectrodes 
% 300622, To be optimized

% Note2 for v8: built on top of v7, after removing the time and psd
% thresold

% Note3 for v9: built on top v8, just prioritizing all the electodes
% instead of only occipitals

% Note3 for v10: built on top v7, prioritizing all the occipital electodes
% and removing teh time thresolds

% no badTrial strings are given

function findBadTrialsWithEEG_v10_forPsdCheck(subjectName,expDate,protocolName,folderSourceString,gridType,badEEGElectrodes,...
...
for iElec=1:numElectrodes
    ...
    numTrials = size(analogData,1);                            % excluding bad eye trials and bad rms electrodes
    ...
    %subplot(8,8,iElec)
    if ~isempty(powerVsFreq)  
        plot(freqVals,log10(powerVsFreq));
    end
    hold on;
    plot(freqVals,log10(meanTrialData),'k','lineWidth',2.5)
    plot(freqVals,log10(tDplus),'m','lineWidth',2.5)
    title(iElec);
    %tmpBadTrialsAll = unique([badTrialsTimeThres;badTrialsFreqThres;badRmsTrials]);
    %     tmpBadTrialsAll = unique([badRmsTrials]);
    %
...
%     amplitude = slopeValsVsFreq{iElec}{1}; % first Output
%     slopeBeta =  slopeValsVsFreq{iElec}{2}; % second Output
% end
%
